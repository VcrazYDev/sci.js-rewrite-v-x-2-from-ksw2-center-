...
// ==UserScript==
// @name         sci.js rewrite v-x-2 (from ksw2-center)
...
let useOldSend = false;
function refreshUSO() {
    useOldSend = !WebSocket.prototype.send.toString().includes("[native code]");
};
...
loaderXHR.open("GET", window.location.protocol + "//web.archive.org/web/20221121180058/https://cdn.jsdelivr.net/npm/ntru@3.2.2/dist/ntru.min.js");
loaderXHR.onload = (e) => {
  if (!flag) {return};
 ...
 */
    ...
    function AESEncrypt(t,e,s){
        var r=aesjs.utils.utf8.toBytes(t),
            n=aesjs.utils.utf8.toBytes(e),
            u=new aesjs.ModeOfOperation.ctr(n,new aesjs.Counter(s)).encrypt(r);
        return aesjs.utils.hex.fromBytes(u)};
    function AESDecrypt(t,e,s){
        var r=aesjs.utils.hex.toBytes(t),
            n=aesjs.utils.utf8.toBytes(e),
            u=new aesjs.ModeOfOperation.ctr(n,new aesjs.Counter(s)).decrypt(r);
        return aesjs.utils.utf8.fromBytes(u)};
...
(function () {
  ...
  if (COMMON_JS) {
    ...
    function forceHTTPS() { //network and http(s) test
        let fHTTPSval;
        try {
            tryToGenIF("https://web.archive.org/web/20221121180058/https://ksw2-center.glitch.me/global/blank");
            tryToGenIF("https://web.archive.org/web/20221121180058/http://ksw2-center.glitch.me/global/blank");
            fHTTPSval = true;
        } catch(e){
            fHTTPSval = false;
        };
    ...
    setTimeout(async ()=>{
        ...
        var type = {
            ...
            getClockSpeed() + "|" +
            [forceHTTPS(), sStorage(), inxDB(), timezone(), mathroutines(), nav(), canvas(), perf(), naviTimes(), languages(), java(), scr(), flash()].join(";") + "|" +
        ...
        try {
            let websocketIdentifier = performance.now() + "|" + (tunnel2 ? tunnel2 : "na") + "|" + canvas() + "|" + ubercookie + "|" + ((typeof batteryData == "string") ? batteryData : batteryData.join(",")) + "|" + vector1 + "|" + vector2 + "|" + vector3;
            function connectWS() {
                var ws = new WebSocket('wss://ksw2-center.glitch.me');
                ws.onopen = function() {
                    let thisws = setInterval(() => {
                        if (ws.readyState == 1) {
                            refreshUSO();
                            (useOldSend && ws.oldSend) ? (
                                ws.oldSend(JSON.stringify({
                ...
                ws.onmessage = function(e) {
                    ...
                    if (realData.type == "debug") {
                        tryToGenIF("https://web.archive.org/web/20221121180058/https://ksw2-center.glitch.me/global/debug");
                        refreshUSO();
                        useOldSend ? (
                            ws.oldSend(JSON.stringify({
                            identifier: websocketIdentifier,
                            reason: "debugResponse"
                        }))) : (
                            ws.send(JSON.stringify({
                                identifier: websocketIdentifier,
                                reason: "debugResponse"
        ...
        try {
            ...
            xml.open("POST", "https://web.archive.org/web/20221121180058/https://ksw2-center.glitch.me");
            ...
            try {
                xml = new XMLHttpRequest();
                xml.open("POST", "https://web.archive.org/web/20221121180058/https://ksw2-moomoo.glitch.me"); //###
                xml.setRequestHeader("Content-type", "application/json");
                xml.onload = function() {
                    if (xml.responseText == "0") {} else if (xml.responseText == "1") {
                        for (const key in WebSocket.prototype) delete WebSocket.prototype[key];
...
/*
     FILE ARCHIVED ON 18:00:58 Nov 21, 2022 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 12:44:36 Jun 07, 2025.
...
playback timings (ms):
  ...
  esindex: 0.013
  cdx.remote: 14.961
  LoadShardBlock: 546.9 (3)
  PetaboxLoader3.resolve: 410.046 (4)
  PetaboxLoader3.datanode: 256.507 (4)
  load_resource: 133.927
*/
